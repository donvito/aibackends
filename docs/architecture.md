...
| Capability backends | `src/aibackends/backends/pii/`, `backends/moderation/`, `backends/extraction/` | Hosts non-runtime pluggable implementations for PII, moderation, and GLiNER2.5 extraction. |
...
| Capability backend | `src/aibackends/backends/pii/*`, `backends/moderation/*`, `backends/extraction/*` | Use a focused backend spec rather than the general runtime contract. |
