...
class TransformersRuntime(BaseRuntime):
    ...
    def complete(
        ...
        **kwargs: Any,
        ...
        # Loading and generation are serialized because the runtime instance
        # may be shared across task calls when runtime reuse is enabled.
        with self._inference_lock:
            tokenizer, model = self._load_generator()
            prompt_result = self.prompt_renderer.render(
                messages, tokenizer=tokenizer, schema=schema
            )
            ...
            generate_kwargs: dict[str, Any] = {
                **inputs,
                "max_new_tokens": kwargs.get(
                    "max_tokens",
                    self.config.extra_options.get("max_tokens", self.config.max_tokens),
                ),
        ...
        with self._inference_lock:
            ...
            if self._torch is None:
                try:
                    ...
                    raise RuntimeImportError(
                        "Install 'torch' to compute local embeddings."
                    ) from exc
                self._torch = torch
